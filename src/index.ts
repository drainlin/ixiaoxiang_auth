  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    try {
      ...
      if (request.method === "GET" && path === "/sync/settings") {
        return withCors(
          await handleGetSyncSettings(request, env),
          env,
          request,
        );
      }
      if (request.method === "PUT" && path === "/sync/settings") {
        return withCors(
          await handlePutSyncSettings(request, env),
          env,
          request,
        );
      }
      if (request.method === "GET" && path === "/sync/libraries") {
        return withCors(
          await handleGetSyncLibraries(request, env),
          env,
          request,
        );
      }
      if (request.method === "PUT" && path === "/sync/libraries") {
        return withCors(
          await handlePutSyncLibraries(request, env),
          env,
          request,
        );
      }
      if (
        ...
        return withCors(
          await handlePasskeyRegisterStart(request, env),
          env,
          request,
        );
      }
      if (
        ...
        return withCors(
          await handlePasskeyRegisterFinish(request, env),
          env,
          request,
        );
      }
      if (request.method === "POST" && path === "/auth/passkey/login/start") {
        return withCors(
          await handlePasskeyLoginStart(request, env),
          env,
          request,
        );
      }
      if (request.method === "POST" && path === "/auth/passkey/login/finish") {
        return withCors(
          await handlePasskeyLoginFinish(request, env),
          env,
          request,
        );
      }
      if (request.method === "GET" && path === "/auth/passkey/settings") {
        return withCors(
          await handleGetPasskeySettings(request, env),
          env,
          request,
        );
      }
      if (request.method === "PATCH" && path === "/auth/passkey/settings") {
        return withCors(
          await handlePatchPasskeySettings(request, env),
          env,
          request,
        );
      }
      if (request.method === "GET" && path === "/auth/passkey/credentials") {
        return withCors(
          await handlePasskeyCredentialsList(request, env),
          env,
          request,
        );
      }
      if (request.method === "PATCH" && path === "/auth/passkey/credential") {
        return withCors(
          await handlePasskeyCredentialUpdate(request, env),
          env,
          request,
        );
      }
      if (request.method === "DELETE" && path === "/auth/passkey/credential") {
        return withCors(
          await handlePasskeyCredentialDelete(request, env),
          env,
          request,
        );
      }
...
async function handleSendCode(request: Request, env: Env): Promise<Response> {
  ...
  await env.OTP_KV.put(
    otpKey(email, purpose, app.appId),
...
async function handleVerifyCode(request: Request, env: Env): Promise<Response> {
  ...
  if (inputHash !== otp.codeHash) {
    ...
    return json(
      { error: "otp_mismatch", attemptsLeft: maxAttempts - attempts - 1 },
      400,
    );
  }
...
async function handleMe(request: Request, env: Env): Promise<Response> {
  ...
  return json({
    ...
    user: {
      ...user,
      display_name:
        displayName || user.email.split("@")[0] || `${app.appName} User`,
...
async function handleGetPasskeySettings(
  request: Request,
  env: Env,
): Promise<Response> {
  const app = resolveAppContext(request, env);
...
async function handlePatchPasskeySettings(
  request: Request,
  env: Env,
): Promise<Response> {
  const app = resolveAppContext(request, env);
...
async function handleGetSyncLibraries(
  request: Request,
  env: Env,
): Promise<Response> {
  const app = resolveAppContext(request, env);
...
async function handlePutSyncLibraries(
  request: Request,
  env: Env,
): Promise<Response> {
  const app = resolveAppContext(request, env);
...
async function replaceLibraryProfiles(
  ...
  await env.DB.prepare(
    "DELETE FROM library_profiles WHERE user_id = ?1 AND app_id = ?2",
  )
    .bind(userId, appId)
    .run();
...
async function handlePasskeyRegisterFinish(
  ...
  const challengeRaw = await env.OTP_KV.get(
    passkeyRegisterChallengeKey(app.appId, authUser.id),
  );
  ...
  const responsePayload = (
    credential as { response?: { transports?: unknown } }
  ).response;
  const transports =
...
async function handlePasskeyCredentialUpdate(
  request: Request,
  env: Env,
): Promise<Response> {
  const app = resolveAppContext(request, env);
...
async function handlePasskeyLoginFinish(
  request: Request,
  env: Env,
): Promise<Response> {
  const app = resolveAppContext(request, env);
  ...
  const challengeRaw = await env.OTP_KV.get(
    passkeyLoginChallengeKey(app.appId, challenge),
  );
  if (!challengeRaw) return json({ error: "passkey_challenge_expired" }, 400);
  const challengeState = parseJson(challengeRaw);
  ...
  const passkeyLoginEnabled = await isPasskeyLoginEnabled(
    env.DB,
...
async function handlePasskeyCredentialDelete(
  request: Request,
  env: Env,
): Promise<Response> {
  const app = resolveAppContext(request, env);
...
async function upsertUserByEmail(db: D1Database, email: string) {
  ...
  const user = await db
    .prepare(
      "SELECT id, email, status, created_at FROM users WHERE email = ?1 LIMIT 1",
    )
    .bind(email)
    .first<{ id: string; email: string; status: string; created_at: number }>();
...
async function sendOtpEmail(args: {
  ...
  const response = args.env.MAIL_GATEWAY
    ? await args.env.MAIL_GATEWAY.fetch(
        "https://mail-gateway.internal/v1/email/send",
        requestInit,
      )
    : await fetch(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `send_email_via_mail_gateway_failed:${response.status}:${detail}`,
    );
  }
}
...
function resolveEmailSubject(env: Env, appId: string, appName: string): string {
  const defaultTemplate =
    `${env.DEFAULT_EMAIL_SUBJECT ?? "{app_name} Verification Code"}`.trim() ||
...
function extractChallengeFromCredential(credential: JsonRecord): string | null {
  ...
  const clientDataJSON =
    `${(response as JsonRecord).clientDataJSON ?? ""}`.trim();
...
async function isPasskeyLoginEnabled(
  db: D1Database,
  userId: string,
): Promise<boolean> {
  const row = await db
    .prepare(
      "SELECT passkey_login_enabled FROM user_profiles WHERE user_id = ?1 LIMIT 1",
    )
    .bind(userId)
...
async function verifyJwt(
  ...
  if (expectedAud) {
    ...
    if (typeof aud === "string") {
      ...
      const matched = aud.some(
        (item) => `${item ?? ""}`.trim() === expectedAud,
      );
...
function resolveAppContextSafe(
  request: Request,
  env: Env,
...
function resolveAppContext(request: Request, env: Env): AppContext {
  ...
  const defaultAppId =
    `${env.DEFAULT_APP_ID ?? ""}`.trim().toLowerCase() || DEFAULT_APP_ID;
  ...
  const appName =
    `${selected.appName ?? env.APP_NAME ?? env.DEFAULT_APP_NAME ?? "CineDock"}`.trim() ||
    "CineDock";
  const appOrigin = normalizeOptionalString(
    selected.appOrigin ?? env.APP_ORIGIN ?? env.DEFAULT_APP_ORIGIN,
  );
  const appBundleId =
    `${selected.appBundleId ?? env.APP_BUNDLE_ID ?? env.DEFAULT_APP_BUNDLE_ID ?? "cn.ixiaoxiang.video"}`.trim() ||
    "cn.ixiaoxiang.video";
  const passkeyRpId = normalizeOptionalString(
    selected.passkeyRpId ?? env.PASSKEY_RP_ID,
  );
  const passkeyRpName =
    `${selected.passkeyRpName ?? env.PASSKEY_RP_NAME ?? appName}`.trim() ||
    appName;
...
function resolveRequestedAppId(request: Request, fallback: string): string {
  const fromHeader = `${request.headers.get("x-app-id") ?? ""}`
    .trim()
    .toLowerCase();
  const fromQuery =
    new URL(request.url).searchParams.get("app_id")?.trim().toLowerCase() ?? "";
